...
class LocalModelRuntime:
    ...
    def semantic_proposal(self, rgb: np.ndarray) -> tuple[np.ndarray | None, dict[str, Any]]:
        ...
        try:
            ...
            tensor, _ = self._image_input(rgb, manifest, model_inputs[0], (1024, 1024))
            proposal = self._run_mask(session, {input_name: tensor}, manifest)
    ...
    def topology_proposal(
        self, rgb: np.ndarray, semantic_alpha: np.ndarray
        ...
        try:
            ...
            if adapter not in {"sam2-conditional-v1", "sam2-mask-prompt-v1"}:
